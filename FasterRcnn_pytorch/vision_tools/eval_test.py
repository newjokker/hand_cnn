...
if __name__=="__main__":
    ...
    from utils.dataset.transforms import valid_transforms
    from config import DefaultConfig 

    config = DefaultConfig
    eval_dataset = VOCDatasetMS(config, root_dir='/home/fbc/Datasets/VOC/VOCdevkit/VOC2007', split='test', use_difficult=False, is_train=False, transforms=valid_transforms)
    ...
    model.load_state_dict(torch.load("exps/20201121__20_04/checkpoints/model_24.pth", map_location=torch.device('cpu')))
    ...
    all_AP=eval_ap_2d(gt_boxes,gt_classes,pred_boxes,pred_classes,pred_scores,0.5,len(config.class_names))
